...
export class TwoAccountsExploit {
  ...
  public constructor(
    private readonly binanceConnector1: BinanceConnector,
    private readonly binanceConnector2: BinanceConnector,
  ) {
  ...
  private async playTheGame(): Promise<void> {
    if (this.iterationCounter === 1) {
      ...
      await this.cleanTheDeskIfNecessary();
      await this.optimizeValueAtRiskOnAccount(
  ...
  private async cleanTheDeskIfNecessary(
    ...
    if (
      marginRatio1 > 83 && this.bnbSpotAccount1 < 0.1 &&
      this.usdtSpotAccount1 < 10
    ) {
      console.log(
        `I close all positions of account 1 because the strategy did not work well this time - marginRatio1: ${marginRatio1}`,
      );
    ...
    if (
      marginRatio2 > 83 && this.bnbSpotAccount2 < 0.1 &&
      this.usdtSpotAccount2 < 10
    ) {
      console.log(
        `I close all positions of account 2 because the strategy did not work well this time - marginRatio2: ${marginRatio2}`,
      );
      await this.closeAllOpenPositions(
        this.accountData2,
        this.binanceConnector2,
  ...
  private async adjustTheAltHedge(
    accountData: any,
    binanceConnector: any,
    position: any,
    pair: string,
    ...
    if (
      ...
      if (
        pnlInPercent > this.closingAt &&
        Number(position.positionAmt <= -hedgeUnit)
      ) {
        console.log(
  ...
  private async enjoyThePerfectHedges(): Promise<void> {
    const batPosition1 =
      this.accountData1.positions.filter((entry: any) =>
        entry.symbol === "BATUSDT"
      )[0];
    const batPosition2 =
      this.accountData2.positions.filter((entry: any) =>
        entry.symbol === "BATUSDT"
      )[0];
    const linkPosition1 =
      this.accountData1.positions.filter((entry: any) =>
        entry.symbol === "LINKUSDT"
      )[0];
    const linkPosition2 =
      this.accountData2.positions.filter((entry: any) =>
        entry.symbol === "LINKUSDT"
  ...
  private async optimizeValueAtRiskOnAccount(
    binanceConnector: any,
    bnbSpotAccount: number,
    usdtSpotAccount: number,
    accountData: any,
  ): Promise<void> {
    const accountId = binanceConnector.getAccountId();
    const marginRatio = Number(accountData.totalMaintMargin) * 100 /
    ...
    console.log(
      `${accountId}: usdtSpot: ${usdtSpotAccount} - bnbSpot: ${bnbSpotAccount} - mr: ${marginRatio} - marginDelta: ${marginDelta}`,
    );

    if (marginRatio > 54 && usdtSpotAccount >= 10) {
      console.log(
        `I transfer ${10} USDT to the USDT Account 1 due to a margin ratio of ${marginRatio}`,
      );
      ...
      const amountToBeConvertedToBNB =
        (usdtSpotAccount - (Number(accountData.totalWalletBalance))) - 2;
      if (amountToBeConvertedToBNB > 0.07) {
        console.log(`I convert ${amountToBeConvertedToBNB} USDT to BNB.`);
        await this.convertToCrypto(
          binanceConnector,
          currentPrices,
          amountToBeConvertedToBNB,
          "BNBUSDT",
          2,
        );
      }
  ...
  private async collectData(): Promise<void> {
    ...
    this.bitcoinPosition1 =
      this.accountData1.positions.filter((entry: any) =>
        entry.symbol === "BTCUSDT"
      )[0];
    this.bitcoinPosition2 =
      this.accountData2.positions.filter((entry: any) =>
        entry.symbol === "BTCUSDT"
      )[0];
  ...
  private isItAGoodTimeToAddToBTC1(): boolean {
    ...
    if (
      ...
      (this.pnlBTC1 < this.addingAt || this.pnlBTC1 > 100)
    ) {
      if (marginRatio1 < 18) {
        return true;
      } else if (marginRatio1 > 27 && marginRatio1 < 36) {
        return true;
      } else {
  ...
  private isItAGoodTimeToReduceBTC1(): boolean {
    const accountMode = this.getAccountMode(this.accountData1);
    if (
  ...
  private isItAGoodTimeToAddToBTC2(): boolean {
    ...
    if (
      ...
      (this.pnlBTC2 < this.addingAt || this.pnlBTC1 > 100)
    ) {
      if (marginRatio2 < 18) {
        return true;
      } else if (marginRatio2 > 27 && marginRatio2 < 36) {
        return true;
      } else {
  ...
  private isItAGoodTimeToReduceBTC2(): boolean {
    ...
    const accountMode = this.getAccountMode(this.accountData2);
  ...
  private async closeAllOpenPositions(
    accountData: any,
    binanceConnector: any,
  ): Promise<void> {
  ...
  private async buyBackAllShortPositions(
    accountData: any,
    binanceConnector: any,
  ) {
    for (let position of accountData.positions) {
