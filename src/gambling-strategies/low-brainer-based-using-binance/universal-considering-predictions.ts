...
// ts-node src/gambling-strategies/low-brainer-based-using-binance/universal-considering-predictions.ts <intervalLengthInSeconds> <size> <threshold> <binanceApiKey> <binanceApiSecret> <pairToBeTraded> <limitRegardingBuyingTheDip> <shortTermDipOrPumpIndicator>
// ts-node src/gambling-strategies/low-brainer-based-using-binance/universal-considering-predictions.ts 5 0.02 1 apiKey apiSecret 0.75 1
// pm2 start -n "considering-predictions" src/gambling-strategies/low-brainer-based-using-binance/universal-considering-predictions.ts -- 60 0.02 5 apiKey apiSecret 0.85 5
...
setInterval(async () => {
    ...
    const predictionLong = await isPredictionLong()
